...
$.fn.extend({
    ...
    fileUp:function(options) {
        ...
        var suppotFile = [ 'jpg','bmp','png','jpeg','gif','tiff'];
            ...
            if(verification){
                function se(userid) {
                    getConfigurationUrl(function(figuration){
                        $.each(that[0].files,function(i,item){
                            var fileNamePreFix = item.name.substring(item.name.lastIndexOf("."),item.name.length),
...
function getConfigurationUrl(callback){
    $.ajax({
        url : ArtJS.server.art+'goodsSite/qiNiu/getConfigurationUrl',
        async : false,
        type : "get",
        dataType : "json",
...
function getQiToken(options){
    ...
    $.ajax({
        async: false,
        url:  ArtJS.server.art+"goodsSite/qiNiu/getUploadToken",
        type: "get",
        dataType: 'json',
        jsonp: 'callback',
        data:{
