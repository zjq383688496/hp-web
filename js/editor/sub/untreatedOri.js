...
;(function(){
	'use strict';
	var normalEditor = angular.module("normalEditor", ["editorHeaderApp","editorSelectorApp","editorGoodsListApp"]);
	...
	$timeout(function () {
		var URL={
			...
			// typeUrl:"/",//获取基本类型URL
			listUrl:"/goodsSite/editor/image/listEditorNotEdited",//获取商品列表URL pageSize=20&editorId=0&orderByType=2&pageNo=1&abbr=CN
			// listUrl:"/",//获取商品列表URL
		...
		$scope.normalEditorCondition={
			//parentUserId:ArtJS.cookie.get('toKen')||"0";//【不传】：默认查询当前登录账号下的副主编【0】：查询所有下下家的小编 用户ID，当前用户下面的用户小编
		};//查询小编接口
		//小编页相关 国际化
		...
		//查询副主编，小编接口
		function getSubNoramlEditors(options){
			// var error=options.error;
			$.ajax({
			    type: "get",
	...
	/*	//审核不通过事件
		// goodsCode=&reason=
		$scope.returnNormal=function(goodsCode,index){
			$scope.editorGoods.goodIndex=index;
			var template='是否退回到小编？';
		...
		/*function loadNavData(){
			...
			$scope.nav.navList=[{code:"goodList",name:lang.nav.pic,href:"/page/"+ArtJS.server.language+"/edit/sub/picture.html"},
			{code:"workList",name:lang.nav.original,href:"/page/"+ArtJS.server.language+"/edit/sub/original.html"},
